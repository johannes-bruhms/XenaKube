...
**Architectural direction - Temporal Identity**: each of 12 face-moves (L/L'/R/R'/F/F'/B/B'/U/U'/D/D') owns a distinct gesture *type* and absolute phrase duration fixed to the cube's color-fixed face identity; K_i / C_i permutation modulates *content* (pitch, timbre, intensity, density) inside that shape. Performer's forward model: you know the *kind* of sound a turn will produce, you don't know the *detail*. Pending phases (A2 solve-anchor, Phase B phrase library, Phase E tier 3 phrase migration): see `docs/todo.md`.
...
| **Face-owned duration** (D74) - face moves resolve phrase duration from `FACE_MAP.durationSec`; K_i vertex duration is neutral fallback only. Keeps face gestures recognizable through changing K/C state. | `src/face-gesture.ts`, `src/vertices.ts`, generated `max/gen_includes.js`, `max/xk_swam.js` `handleVoice`. | `FACE DURATION FAIL face=F ...`; normal voice log includes `dur=Ns(face)`. |
...
*Structural* composition math is TypeScript (`src/`): S4, K_i, C_i, sieve, face-identity, voice / density / intensity decisions, and face-owned phrase duration. *Phrase-level* note generation (pitches inside `foldToRange`, rebow counts, per-complex stochastic contours) currently lives in `max/xk_swam.js`; migration to TS is tracked as Phase B + Phase E tier 3. Max/MSP + SWAM Cello 3 = synthesis via MIDI. TouchDesigner / browser = visuals only.
...
- `/xk/voice` (vertexIdx, complexType, density, intensity, duration): the load-bearing voice trigger. Fires only on real voice transitions (`engine.onVoice`), not per gyro packet. `/xk/face` precedes it for every voice batch; Max uses `FACE_MAP.durationSec` for face-triggered phrase durations and `-` resets stale face state.
...
Each cube turn → cube-algorithm detector → mode-manager (if matched; effect handlers are stubs) → K_i / C_i advances → `/xk/face` on face-moves → voice-engine emits voices → TS phrase planner emits `/xk/phrase/plan` → expression processor supplies continuous controls → state broadcast (OSC + WS). Sequential mode = 1 voice; polyphonic = 8. Engine modes split K_i (`direct`/`diagram`) and C_i (`algorithmic`/`gyro`) independently; freeze halts state advance. Gyro yields tilt / spin / deviation / scramble. Algorithms layer (shorter prefix fires before the longer match). The TS phrase planner (`src/phrase-plan.ts`) and echo auditor (`src/phrase-audit.ts`) are the migration seam off Max-side phrase logic — Max still renders, but every plan is comparable against `/xk/midi/*` echoes via `planId`.

**Full reference**: per-step core loop, voice / engine modes, cube-algorithm book + orientation expansion + half-turn convention + overlap rules, gyro-expression mapping, S4 / sieve / tetra-orbit math — `docs/performance-model.md`. Primary source — `docs/xenakis_nomos_alpha_primary_source.md`. Design rationale — `docs/research_notes.md`.
