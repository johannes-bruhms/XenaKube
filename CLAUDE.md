...
**Temporal Identity model**: each of 12 face-moves owns a distinct gesture *type*. K_i supplies density / intensity / base duration; face signatures reshape duration, envelope, articulation, and velocity contour without prescribing register or pitch direction. Half-turn pairs render short fff punctuation independent of K/face phrase shape while preserving C1 pizzicato, C4 harmonics, and C5-C7 as one short one-direction gliss stroke. Turn rate adds bounded per-complex pressure on top of K_i. The solve edge anchors to beta-cosmo.
...
| **Relay lifecycle shutdown** - last dashboard disconnect, SIGINT/SIGTERM/SIGHUP, and loopback `/api/shutdown` all use one graceful shutdown path: send SWAM + sphere panic, stop the 60 Hz gyro loop, clear relay intervals, close WS/HTTP/OSC/MIDI-echo sockets, then exit with a bounded force timer. Max `stop relay` calls the API and falls back to force-kill; controller exit no longer orphans the relay child. | `relay.js` `gracefulShutdown`; `public/js/transport.js` page-lifecycle close; `max/relay-controller.js` stop/kill handlers. | `[SHUTDOWN] reason`; `[SHUTDOWN] force exit...`; controller `graceful relay shutdown failed` / `force killing relay child...`; static guards in `test/transport-backpressure.test.ts` and `test/max-bridge.test.ts`. |
...
| **Immediate expression seed** — every normal phrase-start CC 11 seed is synchronous and clamped to `ONSET_EXPRESSION_MIN` before first noteon; no proportional attack ramp or low arc/hairpin start may mask turn response. | `src/swam-mapping.ts` `ONSET_EXPRESSION_MIN`; `max/xk_swam.js` `onsetExpressionValue`. | `[ONSET EXPR WARN] ... CC11 is low` when noteon is timely but expression is below `ONSET_EXPRESSION_MIN`. |
...
| **Half-turn punctuation** — rapid identical quarter-turn pairs set `halfTurn=1` on the second voice; audio bypasses normal K/face phrase shape and renders short fff punctuation: C1 pizz, C4 harmonics, C5-C7 one short one-direction gliss stroke each, others bowed dyad. | `src/engine.ts` `detectHalfTurn`; `max/xk_swam.js` `phraseHalfTurn`. | Max log `half-turn punctuation ... tech=pizz|gliss|bowed-dyad`; phrase/audit summaries include `half-turn=1`. |
...
| **C3 retrigger onset reset** - C3 remains internally legato, but never preserves a cross-phrase tail; same-pitch legato retriggers cut the old pitch before the fresh noteOn so the delayed overlap noteOff cannot kill the new attack. | `src/swam-mapping.ts` `LEGATO_COMPLEX`; `max/xk_swam.js` `handleVoice` / `legatoNoteOverlap`. | `C3 ONSET TAIL FAIL ...` if a stale C3 tail survives cancel; normal C3 log includes `samePitchRetriggers=N tailClears=N`. |
...
| **BLE cube stream liveness** — after Bluetooth connect, dashboard must require an open relay WS, unit-normalize GAN gyro events, keep a latest-only relay queue, flush or inline gyro before moves/zeroing, leave live-cube pose to 60 Hz `gyro_tick`, request facelets-only bootstrap, detect move-alive/gyro-stale splits, and relay must re-normalize before Kalman/engine input. | `public/js/main.js` `cubeEventQuaternion` / `flushPendingCubeGyroToRelay` / `requestCubeFacelets`; `relay.js` `extractGyroQuaternion` / `handleBleGyroPayload`. | `[CUBE RELAY OFFLINE]`, `[CUBE STREAM FAIL]`, `[CUBE GYRO FAIL]`, `[CUBE GYRO FORWARD FAIL]`, `[CUBE GYRO STALE]`, `[CUBE MOVE FAIL]`, `[BLE GYRO DROP]`; static guard in `test/transport-backpressure.test.ts`. |
...
| **Ghost snap + active C stay local in beta-cosmo** — ghost orientation uses unit `state.snapQuat`; C dots/labels stay fixed; beta active C = `C{activeVertex+1}`, not the alpha-table C at that slot. | `src/quaternion.ts` S4 snap quats from `group.ts`; `src/complexes.ts` fixed beta C1..C8; `public/js/cube-scene.js`. | `[GHOST SNAP FAIL]`, `[GHOST ACTIVE SLOT FAIL]`, `[GHOST TURN LEAK FAIL]`. |
...
Instantiates `XenaKubeEngine` (default `beta-cosmo`; set `XK_COSMO=alpha-cosmo` to boot the historical walk path), serves `public/dashboard.html` on `:3000`, receives cube events via WS from the browser. Performance sessions start it from Max via `node.script relay-controller.js` (`relay` / `start relay`); `npx tsx relay.js` is the direct dev/debug path. Deps: `node-osc`, `ws`, `tsx`.
...
- **Lifecycle**: immediate auto-shutdown after the last dashboard client disconnects (override with `XK_SHUTDOWN_DELAY_MS` only when a refresh grace period is deliberate). Shutdown is idempotent: relay sends `/xk/panic` + sphere panic, stops the 60 Hz gyro loop, clears lifecycle intervals, closes WS/HTTP/OSC/MIDI-echo sockets, and exits under `SHUTDOWN_FORCE_EXIT_MS`. Loopback `POST /api/shutdown` is for Max `stop relay`; signal handlers use the same path.
...
`dashboard.html` at `http://localhost:3000`. Full-viewport HUD: cube canvas, adaptive contrast SVG, transparent overlays, phrase-triggered K/C cards anchored to the active ghost C, MIDI brush canvas above an optional actual-audio spectrogram canvas. Modular layout under `public/js/{cube-scene,rolling-score,spectrum-score,performance-recorder,triangle,sieve,state-ui,transport,settings-sync,main}.js`; styles in `public/css/main.css`. Quality picker toggles cube bloom; bottom-right settings panels tune spectrogram, cube/triangle colors, and persistent dashboard presets. Active settings live in `data/dashboard-settings.json`; named snapshots live in `data/dashboard-presets.json`. C vertices use the MIDI brush colors/textures; the cube colors panel edits K vertices, label/card text, wires, halos, and triangle lines. Optional `public/interruption/` stays behind `?intrusions=1`. CSS `--roll-*-inset` MUST equal JS `ROLL_*_INSET_PX` (`assertPitchAxis`). Rolling-score owns MIDI brushes/gliss chains; spectrum-score consumes only Max `/xk/spectrum/frame` data and is off by default.
