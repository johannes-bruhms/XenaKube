...
**Architectural direction - Temporal Identity**: each of 12 face-moves (L/L'/R/R'/F/F'/B/B'/U/U'/D/D') owns a distinct gesture *type*. K_i supplies density, intensity, and base duration; face signatures reshape duration/register/envelope/motion without replacing the material. Turn rate adds bounded per-complex pressure on top of the K_i baseline, not a replacement control layer. Performer's forward model: you know the *kind* of sound a turn will produce, not every detail. Pending phases (A2 solve-anchor, Phase B phrase library, Phase E tier 3 phrase migration): see `docs/todo.md`.
...
| **Canonical-pose face-letter remap** — assumes performer holds cube red-front, white-top at connect. GAN reports moves in factory sticker frame (R=red, F=green, …); `MOVE_REMAP = {R↔L, F↔B, U/D unchanged}` (180° around U) applied at relay realigns engine + dashboard + `/xk/face` OSC + algorithm-buffer to user-pose geometry. Algorithm detection stays orientation-invariant (24 pre-expanded variants in `cube-algorithm.ts`). Three mirrored copies (`relay.js`, `cube-scene.js`, `main.js`) MUST stay in sync. | `relay.js` `MOVE_REMAP` (WS `move` handler, before `engine.onTurn` + `OSC.GAN_TURN`); `cube-scene.js` `CANONICAL_REMAP` + `assertCubeAlignment()` in `applyConnectView`; `main.js` `CANONICAL_REMAP` + `verifyMoveRemap` on first `state.move` per connect. | `[CUBE ALIGN FAIL]` (geometric, on connect — CUBE_VERTS/camera/dashboard-table drift) and `[CUBE REMAP FAIL] gan='X' expected='Y' got='Z'` (per-move — relay-table drift). |
...
`dashboard.html` at `http://localhost:3000`. Full-viewport HUD: `#cube-canvas` fills the window, all UI floats as transparent overlays, full-viewport rolling-score canvas paints behind the cube. Phase-2 modular layout under `public/js/{cube-scene,rolling-score,triangle,sieve,state-ui,transport,main}.js`; styles in `public/css/main.css`. Cube scene runs through an `EffectComposer` (RenderPass → UnrealBloomPass → OutputPass) with `ACESFilmicToneMapping`; `setQuality('low'|'med'|'high')` toggles between direct render and the bloom chain via the bottom-right picker (default Med). Optional `public/interruption/` overlay is disabled unless `?intrusions=1` and is wired only from `main.js`. CSS `--roll-*-inset` MUST equal JS `ROLL_*_INSET_PX` (`assertPitchAxis` enforces). Per-complex procedural brushes (spatter / wash / watercolor / airbrush / chalk-grit) for C1-4/C8; C5/C6/C7 gliss chains drawn as single stroked Path2D.
...
- `/xk/voice` (vertexIdx, complexType, density, intensity, duration): the load-bearing voice trigger. Fires only on real voice transitions (`engine.onVoice`), not per gyro packet. `/xk/face` precedes it for every voice batch; Max multiplies incoming K duration by `FACE_MAP.durationMult` and `-` resets stale face state.
