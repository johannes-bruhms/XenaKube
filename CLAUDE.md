...
| **Slide-vs-leap classifier consistency** — white-line and rolling-chain must classify the same MIDI pattern identically. A leap (clean `noteoff → 50 ms gap → noteon`, bridge `LEAP_GAP_MS`) must result in NO line animation AND a chain break. A slide (overlapping noteons, bridge `glissNote`) must result in a line retarget AND a chain continue. | `public/dashboard.html` white-line: `_findGlissLine(voice, complex)` — slide iff a same-(voice,complex) lineNote is alive at noteon time. Rolling chain: `GLISS_GAP_MS = 25` ms threshold in `buildGlissChains` (gap > 25 → new chain). Both thresholds are below the bridge's 50 ms `LEAP_GAP_MS` and above its negative-gap overlap. | Structural: dashboard reads the bridge's actual gap pattern, doesn't try to classify ambiguously. White-line uses presence of live entry; rolling chain uses gap-magnitude. | No dedicated runtime telemetry yet — the symptom (leap incorrectly drawn as a slant slide) is visually obvious. If future bridge timing changes the gap, both `_findGlissLine` (no parameter) and `GLISS_GAP_MS` need a coordinated update. |
