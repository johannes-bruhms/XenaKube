...
| **Gliss-line trajectory equals drawn-note trajectory** — white triangle leg endpoint Y at any time t equals rolling-score gliss-chain pitch curve Y at the same t, ≤1 px drift. Both must use the same easing (`easeSlide` cubic smoothstep), the same per-complex portamento times (`PORTAMENTO_MS_PER_SEMITONE = { 5: 50, 6: 80, 7: 115 }`), and the same retarget rule (mid-slide noteons hand off the *currently displayed* pitch, not the discrete prev `toPitch`). | White line: `public/js/triangle.js` `easeSlide` + `predictGlissDuration` + `_displayedPitch`. Rolling chain: `public/js/rolling-score.js` `_glissEase` + `_glissChainDur` + `_buildGlissSegments` + `_glissPitchAt`. Both consume `PORTAMENTO_MS_PER_SEMITONE` / `GLISS_PORTAMENTO_MS_PER_SEMITONE` from `public/js/constants.js`. Phase 4.2 (`docs/todo.md`) plans consolidation into a unified pitch-trajectory model that drives both sides — until then, the two segment models drift apart on any refactor that touches one but not the other. | Structural: same constants table (constants.js), byte-identical easing function (cubic smoothstep). Both code paths derive `pitchAt(t)` from a segment list with identical interpolation. Cross-module wiring: `rollingScore.init({ getActiveGlissLineDisplay: triangle.getActiveGlissLineDisplay })` in `public/js/main.js` is the ONLY place these two segment models meet — without that wire, the assertion silently no-ops (see "Cross-module init wiring" row below). | `assertGlissSync` in `public/js/rolling-score.js` runs ~1 Hz when any active gliss line exists. Pulls the triangle's currently-displayed gliss pitch via the wired `getActiveGlissLineDisplay(now)`; computes the rolling-chain's segment-model pitch at the same `now`; both go through the SAME `midiToY` so drift is purely trajectory-model drift. `console.error('GLISS SYNC FAIL voice=V complex=C line=Ypx chain=Ypx drift=Dpx linePitch=L chainPitch=C')` on miss. |
