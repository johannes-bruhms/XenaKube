...
**Architectural direction — Temporal Identity**: each of 12 face-moves owns a distinct gesture *type*. K_i supplies density / intensity / base duration; face signatures reshape duration, envelope, articulation, and velocity contour without prescribing register or pitch direction. Rapid same-move pairs become half-turn punctuation: short, loud, assertive, independent of the normal K/C/face phrase shape. Turn rate adds bounded per-complex pressure on top of K_i, not a replacement control layer. The solve edge anchors back to beta-cosmo. Pending phases (Phase B phrase library, Phase E tier 3): `docs/todo.md`.
...
| `AGENTS.md` + subtree `AGENTS.md` | Repo-local agent operating instructions distilled from this file. Keep in sync when workflow / source-of-truth / invariants change. |
...
|---|---|---|---|
| **Tracked numeric constants** | A constant value changes in code while a stale value lingers in any tracked Markdown. Full list in `scripts/check-doc-sizes.mjs`. | `scripts/check-doc-sizes.mjs` constant-sync pass: parses each tracked constant from its authoritative source and asserts every co-located numeric literal in every Markdown file matches. | Reword the doc to reference the constant by name OR update the literal. The `bang()` boot log is the authoritative surface for paired tunables. |
...
| **Immediate first gliss** (D43 + D53) — first `glissStep` fires at the per-complex first-gliss constant (`FIRST_GLISS_MS` for C5, `FIRST_GLISS_MS_C6` for C6, `FIRST_GLISS_MS_C7` for C7), independent of duration. | `max/xk_swam.js` `FIRST_GLISS_MS*`, `phraseC5/6/7`; `src/phrase-plan.ts` mirror. | Covered by D42 `GLISS FAIL`; static guard in `test/max-bridge.test.ts`. |
| **Turn-to-noteon latency telemetry** — every normal C1-C8 phrase pins the first planned main noteon to 0 ms; `/xk/voice` should get that non-companion `/xk/midi/noteon` quickly. Measures bridge echo only, not SWAM attack or Auto Poly look-ahead. | `src/phrase-plan.ts` / `max/xk_swam.js` first-onset schedulers; `relay.js` latency probe around `engine.onVoice` and MIDI echo. | `[LATENCY WARN]` ≥150 ms, `[LATENCY FAIL]` ≥250 ms / no echo in 3000 ms; logs `planFirst` / `planOverrun`; tests in `test/phrase-plan.test.ts` / `test/max-bridge.test.ts`. |
