...
`dashboard.html` at `http://localhost:3000`. Full-viewport HUD: `#cube-canvas` fills the window, all UI floats as transparent overlays, full-viewport rolling-score canvas paints behind the cube. Phase-2 modular layout under `public/js/{cube-scene,rolling-score,triangle,sieve,state-ui,transport,main}.js`; styles in `public/css/main.css`. Optional `public/interruption/` overlay is disabled unless `?intrusions=1` and is wired only from `main.js`. CSS `--roll-*-inset` MUST equal JS `ROLL_*_INSET_PX` (`assertPitchAxis` enforces). Per-complex procedural brushes (spatter / wash / watercolor / airbrush / chalk-grit) for C1-4/C8; C5/C6/C7 gliss chains drawn as single stroked Path2D.
