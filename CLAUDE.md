...
| **Immediate first gliss** (D43) — the first `glissStep` of every C5/C6/C7 voice fires at `FIRST_GLISS_MS` (150 ms) from phrase start, independent of duration. A stolen-short voice (fast turn at ~300 ms) still delivers an audible slide instead of playing as a sustained single note. | `max/xk_swam.js` `FIRST_GLISS_MS` constant; `phraseC5` / `phraseC6` / `phraseC7` schedulers | Structural: phrases schedule idx 0 (C5, C7) or idx 1 (C6) at `FIRST_GLISS_MS` as a hard constant; remaining slides distribute through the phrase tail. `scheduleRelease`'s 200 ms floor is always > `FIRST_GLISS_MS`, so the first slide never races the release ramp even at tiny durations. | No dedicated failure log — the D42 "GLISS FAIL overlaps=0" counter catches any regression that would affect this too. Slide latency is a tunable, not an invariant. |
...
| **Leap-alternation with tolerance** (D48 + D51) — leaps (cross-string `leapStep`) are capped at `MAX_CONSECUTIVE_LEAPS = 2` consecutive runs. Once that count is reached, the next would-be cross-string outcome is nudged onto source's strings to force a slide. Allows leap → leap → forced-slide chains (preserves pre-D48 leap-cluster density that gave wild gliss its rapid-fire attack character) while preventing 3+ leap runs that read as "consecutive non-gliss notes." Phrase anchor no longer seeds the counter — first event after the anchor is fully natural so dramatic anchor → leap openings remain available. C6 (`MIN_LEAP = 1`) and C7 (small drifts) are mostly same-string already; the rule is a no-op for them. | `max/xk_swam.js` `glissStep` (counter-gated nudge) + `nudgeToSameString(sourcePitch, preferTarget, minLeap)` (picks a same-string target ≥ minLeap from source; candidate strings shuffled when source sits in a multi-string overlap zone so trajectory isn't biased toward one string) + `handleVoice` zeros `consecutiveLeap*` counters at phrase start. `MAX_CONSECUTIVE_LEAPS` tunable: 1 (strict alternation, original D48), 2 (current — balanced), 3+ (looser, more leap-clusters). | Structural: `glissStep` checks `(inst.consecutiveLeapCurrent | 0) >= MAX_CONSECUTIVE_LEAPS && !sameString(...)` and nudges target to same-string before dispatch. `nudgeToSameString` returns null only when source is at the extreme of every string it sits on with no minLeap headroom in either direction (rare — extreme cello-range corners). | Per-phrase log in `scheduleRelease` natural-end task tracks `inst.consecutiveLeapMax`. `consecutiveLeapMax > MAX_CONSECUTIVE_LEAPS` (run exceeded tolerance despite the nudge) promotes to `GLISS RUN FAIL inst N CX face=F slides=S leaps=L consecLeapMax=M dur=D`. Otherwise the existing per-phrase `slides=S leaps=L` line is unchanged. If GLISS RUN FAIL is frequent, narrow `pickPitch`'s C5 register or lower `MIN_LEAP`. |
...
- **Face → gesture shape** (Phase A1 Temporal Identity): `/xk/face <face>` populates `state.face*` from `FACE_MAP` (durationBias / registerBias / envelope / articulation / motion); `handleVoice` snapshots onto the allocated `inst` so face changes don't retroactively reshape an in-flight phrase. `phraseC1..C8` consult `faceShapedCount(inst, lo, hi, forGliss)`: `ENV_PROFILE.isSingle` (pluck/stab/drone) collapses non-gliss complexes to a single note; gliss complexes (C5/C6/C7) get `forGliss=true` with a hard minimum of 1 subsequent slide (see Bridge Invariants). `ENV_PROFILE.countMult` (burst ×1.8) thickens density; `stepVelScale` shapes per-step velocity (swell cresc, fade dim, stab/burst accent-first). `commitSieveWalk(count, motion)` honours the face's `up`/`down` to force sieve-walker direction on C2/C6. `scheduleExprEnvelope` scales attack/peak/sustain by `ENV_PROFILE.{attackCoef, peakCoef, sustainCoef}`.
