...
| `/xk/voice` | int, int, float, string, float | vertexIdx, complexType, density, intensity, K_i base duration; face moves multiply by `FACE_MAP.durationMult` in Max |
| `/xk/phrase/plan` | int, int, string, float, int, int, int, int | TS shadow phrase summary. Args: planId, complexType, face or `-`, durationSec, eventCount, noteOnCount, bendStepCount, companionNoteOnCount. Sent immediately before the matching `/xk/voice` so Max can stamp all resulting MIDI echoes with the plan id. |
