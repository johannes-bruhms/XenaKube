...
| `/xk/sphere/strike` | string, float, float, int, int | relay → Max (57121). Mandala-cosmo only. Args: sampleName (canonical from `src/gamelan-manifest.ts`), gain (0..1), pan (-1..1), voiceSteal (0|1; 1 = silence other voices in same instrument class before play), strikeId (monotonic; pairs with /xk/sphere/echo for D75 audit). Purely additive — never replaces /xk/voice, never mutates SWAM state. |
| `/xk/sphere/panic` | (none) | relay → Max (57121). Flush every sphere voice; fired on cosmology change, reset, WS disconnect, and `/xk/panic`. Mirrors the D76 panic invariant in `max/xk_sphere.js`. |
| `/xk/sphere/echo` | string, int, float, int | Max → relay (57122). D75 audit pair to /xk/sphere/strike. Args: sampleName, strikeId, gainActual, voiceSlotActiveCount. Late or missing echoes log `[SPHERE STRIKE FAIL]` after a 400 ms timeout (suppressed until at least one echo arrives, so the audit doesn't spam when the sphere chain isn't loaded). |
| `/xk/sphere/loaded` | int, int, string | Max → relay (57122). One-shot on `bang()`. Args: loadedCount, expectedCount, tuningHash. Relay logs `[SPHERE LOAD]`. Hash mismatch with `GAMELAN_TUNING_HASH` in `src/gamelan-tuning.ts` indicates a Scala edit that wasn't propagated through `npm run gen:max` (D78). |
