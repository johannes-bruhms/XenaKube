...
- [x] 2.5 — Extract `js/cube-scene.js`. Owns: Three.js scene, live K-cube + ghost C-cube + K↔C 3D line, gizmo (cam/live/ghost rotate target), all per-frame animations (gyro live rotation, ghost SLERP, active-step LERP, vertex/ghost-vertex perm-change LERPs, active K/C elastic attraction, active-label merge/fade), camera auto-fit, gyro zero (manual + auto on connect-view), and active-pair screen projection for HUD anchoring. Exports `init({ onAutoZero })` / `setCubeQuat` / `update(state)` / `applyConnectView` / `revertConnectView` / `zeroGyro` / `setGhostScale` / `getActiveKWorldPos` / `getCWorldPos` / `getActiveCardAnchorScreenPos` / `getCamera`. The auto-zero callback fires once after the connect-view orbit lands so main.js can mirror the zero to the relay (matches the Zero button's behaviour). 865 lines.
...
- [x] 2.8 — Extract `js/state-ui.js`. Owns: state-panel rows (face / active voice / S4 element / path / step / snap / cycle / orbit / scramble / perm), mode badges (palette / voice / regime / solved), phrase-triggered floating K/C cards (vertex-grid + complex-grid anchored by cube-scene's active-label merge projection), seq pips, algorithm buffer + toast + partial-matches panel, expression panel (tilt / spin / dev / scramble), recent-moves FIFO, `FACE_SIG` HUD mirror, `COMPLEX_SHORT`. Exports `init({ getActiveCardAnchorScreenPos })` / `update(state, move)` / `handleAlgorithmEvent` / `setSolvedBadge` / `updateExpression(quat, dev)` / `setAlgorithmBook`. 423 lines.
...
- [x] Tune bloom radius / threshold / intensity in browser. Med (restrained: strength 0.32 / radius 0.30 / threshold 0.90) and High (wide: 1.45 / 1.05 / 0.42) verified in-session 2026-05-16; halo opacity, live/ghost edge opacity, adaptive-line opacity, and active-ring size also tied to tier.
...
**Acceptance**: cube has a perceptible glow without performance regression; quality picker works; CLAUDE.md + docs/dashboard-architecture.md describe the pipeline. **Phase complete 2026-05-16.**
...
## Deferred during v2; still pending

These were originally framed as reintegration follow-ups for a v2 merge-back into the main XenaKube tree. v2 is now effectively the main line, so there is no merge-back gate; the work remains pending.

- [ ] **D59 finish — zero out `MAX_LEAPS_BY_COMPLEX`**. D72 shipped pitchbend-driven cross-string slides, but the D58 per-complex leap caps (`{ 5: 1, 6: 0, 7: 0 }`) remain as a belt-and-suspenders fallback. Once pitchbend is trusted to handle every interval within `PITCHBEND_RANGE_SEMI`, set all entries to 0 and retire the leap path. Also covers the slow C7 drift design from `docs/research_notes.md` § Cross-String Pitchbend Slides. Touches: bridge `bendStep` / `rampPitchbend` / leap retirement, `/xk/midi/bendstep` OSC echo, dashboard `triangle.js` / `rolling-score.js`, SWAM preset Pitchbend Range target ±12 (currently ±24 per D64), and the D48/D59 invariant docs. Estimated effort: ~5 hours.
- [ ] **Phase 1.2 pitch-axis threshold tightening**. `PITCH_AXIS_DRIFT_THRESHOLD_PX` is currently 2 CSS px to absorb subpixel noise from canvas-vs-CSS rounding. Proper fix: source `midiToY` from CSS-derived `innerHeight - ROLL_TOP_INSET_PX - ROLL_BOTTOM_INSET_PX` (unrounded) instead of canvas-derived `(canvas.height - (ROLL_TOP_INSET_PX + ROLL_BOTTOM_INSET_PX) × dpr) / dpr`, so the rolling score and sieve share the same source of truth. Once both paths agree, drop the threshold to 1 CSS px. Requires auditing every `midiToY` call-site because the signature should drop the `h` parameter.
- [ ] **Wild-gliss leap drawn as gliss**. Diagnostic commit `841a136` flagged UDP reorder as the likely cause. Fix: bridge emits `/xk/midi/leap <voice> <complex>` OSC marker between `noteoff(old)` and the scheduled `noteon(new)` inside `leapStep`. Dashboard sets a pending-leap-break flag for that voice/complex; the next noteon force-sets `chainStart=true` regardless of UDP arrival order. Marker is order-independent at the OSC level.
- [ ] **Workflow tooling** — `maxmsp` MCP, performance-mode coordination, and other dev-environment differences that surfaced during v2 work but did not block it.
