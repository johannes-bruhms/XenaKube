...
**Tier 2 — Literal echo** *(landed 2026-04-23 — tier 1 replaced outright)*. The `voice`-event archetypal strip from tier 1 didn't survive contact with polyphonic mode: every turn rendered the same 8-pitch-class cluster because `pickNotationPitch` packed all 8 vertices into one octave. Rather than patch tier 1's pitch picker, we jumped straight to the faithful-transcription path: Max bridge mirrors every `noteOn` / `noteOff` to the relay over UDP 57122 (`/xk/midi/noteon|noteoff|panic`), relay forwards as `midi_echo` WS messages, dashboard pairs on `(voice,pitch)`, measures duration on the browser clock, quantises to VexFlow `16/8/q/h/w` at 180/380/900/2500 ms cutoffs. Each phrase rebow shows as a separate note; keyswitches excluded; dynamics annotated every 4th note from MIDI velocity; sharps-only accidentals; stuck-note sweeper force-completes entries >45 s.

- [x] Max-side echo: `ECHO_OUTLET = 2` in `xk_swam.js`; `emitEchoNote()` fires from `noteOn` / `noteOff` (keyswitches bypass); panic echo from `bang()`.
- [ ] **Patch-side: wire `[udpsend 127.0.0.1 57122]` to outlet 3 of `[v8 xk_swam.js]` in `max/xenakube_cello.maxpat`** — one-object add (not a repo edit, the patch is binary-ish JSON but live-edited in Max). Without this wire the echo outlet fires into nowhere and the rolling score stays empty; everything else in the code path is inert.
- [x] Relay-side: `node-osc` `Server` on `MIDI_ECHO_PORT = 57122`; per-packet forward as `{type:'midi_echo', data:{kind,voice,pitch,velocity}}`.
- [x] Dashboard-side: tier-1 transcription ripped; MIDI pairing model with rolling 16-note buffer; 45 s pending-note sweep.
- [ ] Post-playtest: tune the duration-cutoff table against measured phrase rhythms (default assumes ~90 BPM; a fast sexy-move may render all rebows as sixteenths).
- [ ] Consider a "currently ringing" rendering pass — show pending noteons at the right edge in a lighter opacity so drones don't visually disappear until they release (tier 2.5).
