...
> **D75 (2026-05-05): pitchbend channel off-by-one.** Screenshot `docs/temp-screenshots/2..png` showed C5 wild-gliss telemetry with `bends=8` but straight magenta note blocks and no audible gliss. Root cause: `emitPitchbend` used `0xE0 + MIDI_CH`, while note/CC helpers use one-based `MIDI_CH` as `base + (MIDI_CH - 1)`. With `MIDI_CH = 1`, notes went to channel 1 and pitchbend went to channel 2. Fix: add `statusPitchbend(ch)`, route `emitPitchbend` through it, add `PITCHBEND CHANNEL FAIL` telemetry, and add `test/max-bridge.test.ts` to reject the old status expression.

> **D76 (2026-05-05): C5 wild-gliss bend integrity.** The prior D75 fix corrected a shared pitchbend status bug, but C5 still had two phrase-local hazards. The active `phraseC5` referenced `wildAccent` without defining it in that function (the only definition lived in the preserved legacy comment block), so scheduled C5 tasks could abort before `glissStep` and leave straight anchors. It also added fresh per-rebow companions immediately after `glissStep`; those noteOns land during `bendPending` as louder straight attacks and can mask the source bend. Fix: define and guard `wildAccent` in active `phraseC5`, make C5 companion anchor-only and clear it before the first bend, suppress/log any `maybeDoubleStop` while a bend is pending (`BEND COMPANION FAIL`), mirror the change in `src/phrase-plan.ts`, and add static tests for the active Max function plus TS companion count.
...
Same-day continuation of the D54–D64 wild-gliss stabilization thread, focused on the rolling-score / triangle visual layer. Each D-code closes one residual line ↔ chain disagreement. CLAUDE.md Visual Invariants row 3 ("Gliss-line trajectory equals drawn-note trajectory") accumulates D65 / D66 / D67 / D68 sub-invariants; CHANGELOG has dated entries per D-code.

- **D65** — `_buildGlissSegments` bend-segment `p0` inherits `_glissPitchAt(t_bend, prior_segs)` (audible at bend-start) instead of the bend's stated `fromPitch` from the bridge. The bridge's `fromPitch` is the LOGICAL source noteOn pitch; when prior portamento overlaps with the bend (e.g., 9-semi C5 slide ≈ 450 ms but next event fires at MIN_GLISS_SPACING_MS = 200 ms), the audible at bend-start differs from stated source. Line model already uses `_displayedPitch` (audible); chain now matches. Both sides agree to floating-point precision; `assertGlissSync` returns to catching architectural drift only.
- **D66** — `GLISS_SLIDE_MAX_DUR_MS = 195 ms` cap on slide segment dur (both `_glissChainDur` and `predictGlissDuration`). Pre-D66 wild gliss with 28-semi swing computed `28 × 50 = 1400 ms` ideal dur; at 200 ms event spacing the segment walked only 14 % of arc (eased to ~6 %) before next event override → visual amplitude collapsed from 28 semis to ~1.6 semis even though SWAM reached full target audibly. Cap restores visual amplitude.
- **D67** — non-last slide segments use gap-based dur (chain reads `next.onsetMs - this.onsetMs - 5`; line uses `lastEventByVk` predictor `now - prev_event_time - 5`). Eliminates the post-D66 horizontal plateaus on sparse phrases (12 events in 4 s ≈ 335 ms spacing → previously 140 ms plateaus per event boundary). `assertGlissSync` threshold raised 1 → 5 px to absorb first-slide model gap (anchor → first drift = 150 ms vs subsequent ≈ 335 ms). `GLISS_SLIDE_MAX_DUR_MS` retained as a fallback for the LAST/in-flight segment only.
- **D68** — extends D67 to the in-flight last segment. Pre-D68 the chain's last-node fallback used `min(GLISS_SLIDE_MAX_DUR_MS, interval × perSemi)` (cap-based) while the line used `max(80, lastGap - 5)` (gap-based) — model asymmetry that plateaued the chain at p1 for `(gap − 195) ms` until the next noteon arrived and D67 retroactively rebuilt the segment. User-visible "blocky → transforms" symptom on C5 wild (1.4× line width amplifies any residual flatness). Fix: chain's last-seg fallback now mirrors triangle.js exactly — same `lastGap` predictor, same gate, same floor. First-slide path (no prior gap) still uses `predictGlissDuration` so both sides match on the very first slide of a phrase.
- Recurring-Bug Discipline trail: a `CHAIN PLATEAU FAIL` canary was added alongside D68 to catch the asymmetric-predictor class. False-positived on legitimate phrase-tail sustain (short C7 phrases where `FIRST_GLISS_MS_C7 = 30 ms` falls below the predictor's 50 ms gate, so both line and chain correctly fall back to cap-based — plateau ≠ defect). Canary removed; `assertGlissSync` covers the residual class. CLAUDE.md row 3 carries a one-line warning so a future refactor doesn't re-add the same flawed shape.

Cosmetic / palette work landed alongside (CHANGELOG entries this same day): C5/C7 hue split (gliss family no longer three values of the same purple), per-K vertex palette (8 unique warm hues replace the binary tetra split), recoloured 8-unique ghost (C-vertex) palette, per-complex gliss line width multiplier, C4 airbrush 10× density, white active-vertex glow ring, and Display-P3 wide-gamut auto-promotion for the rolling-score brush context.

### D54–D64 — Wild gliss stabilization + cross-string slides via pitchbend *(RESOLVED 2026-04-30)*

Single-day iteration log (2026-04-30) shipping the cross-string pitchbend slide design and the surrounding bridge stability work. Full design in `docs/research_notes.md` § "Cross-String Pitchbend Slides (D59 — design)" with the "What actually shipped (D59 → D64)" subsection at the end. CHANGELOG has dated entries per D-code. Headline summary:

- **D54** — `handleExprTilt` was the only `handleExpr*` modulator without smoothing; gyro static-pose noise produced a 30 Hz CC 16 (Bow Position) flicker between adjacent integers, audible as a sub-vibrato bow buzz on sustained voices. Added `state.tiltEMA` + `TILT_EMA_ALPHA = 0.05`. Per-instance flap counters log per-phrase; `BOW POS FLAP` failure log when reversal rate exceeds threshold.
- **D55** — Diagnostic-only `PORT TIME FAIL` assertion at every `glissNote` entry. Confirms `inst.ccCache[CC.PORTAMENTO_TIME / PORTAMENTO_ON / BOW_POLYPHONY]` matches what SWAM should be in for the slide to engage. Catches the SWAM v3 "wiggle through 0" race silently leaving CC 5 = 0.
- **D56** — `chainStart` flag preserved across note finalisation in `rolling-score.js`'s `noteOff` and stuck-note watchdog. Without preservation, `_findActiveGlissChain` reverted to gap-only classification on finished entries → voice-steal-driven phrase boundaries (gap ≈ 0 ms) silently chained the new phrase onto the old phrase's segments → multi-semitone `GLISS SYNC FAIL`. The Phase 1 invariant fired loudly on the real bug it was built for.
- **D57** — C1 pizz uses static `ccForce(CC.EXPRESSION, peakExpr)` at phrase start (no envelope). Pizz clusters fire all plucks in the first ~200 ms; the legacy phrase-spanning 3-stage CC 11 envelope was modulating CC 11 long after the plucks had already played at attack-time velocity → felt meaningless. Per-pluck velocity (already varying via `stepVelScale`) is the dynamic now.
- **D58** — `MAX_LEAPS_BY_COMPLEX = { 5: 1, 6: 0, 7: 0 }` per-complex leap tolerance via `maxLeapsFor(complexType)`. C5 wild gliss keeps its punch (1 leap allowed); C6/C7 always slide via same-string nudge. Replaces the unified `MAX_CONSECUTIVE_LEAPS = 1` that allowed 50/50 slide/leap alternation on high-anchored phrases descending across the 57 (A-string lower bound).
- **D59 (initial design)** — `bendStep` cross-string slides via pitchbend wheel on the held source note. `glissStep` three-way dispatch: same-string → `glissNote` (portamento), cross-string + within `PITCHBEND_RANGE_SEMI` → `bendStep`, cross-string + over range → `leapStep`. Atomic transition at ramp end (bend reset → noteOff source → noteOn target). New OSC `/xk/midi/bendstep` echo. Dashboard `triangle.bendStep` + `rolling-score.bendStep`. Initial paired tunable `PITCHBEND_RANGE_SEMI = 48`.
- **D60** — three real bugs in the D59 implementation, all critical:
  - D60.1: `humanPitch` is a 10 %-chance ±1 semi shift, not the small jitter D59 design assumed. `bendStep`'s fresh `humanPitch(sourcePitch)` mismatched `inst.activeNotes` → noteOff hit a pitch SWAM didn't have → stale notes ringing forever, queues piling up. Fix: `Math.round`-tolerant lookup of `heldSource` in `inst.activeNotes`.
  - D60.2: bend duration overran `MIN_GLISS_SPACING_MS`. C5 8-semi bend at 50 ms/semi = 400 ms, events space 200 ms. Pitchbend offset leaked into next event's `noteOn`. Fix: `_bendDur` capped at `MIN_GLISS_SPACING_MS - BEND_DUR_MARGIN_MS` (initially 50 ms margin).
  - D60.3: dashboard rendered the bend's future as a horizontal line ahead of "now". Fix: clamp `x1` at `rightEdge` for in-flight bends.
  - D60.4: chain-grace pitch tolerance ±1 for `humanPitch` jitter on bend's target noteOn.
- **D61** — `chainStart` parameter wired through `main.js handleMidiEcho` to `triangle.noteOn` for gliss complexes. Splices any preserved (bend-grace) line and creates fresh on `chainStart=true`. Without this, leaps that fired during bend-grace were silently retargeted as slides on the white line.
- **D62.1** — bend chain-grace consumed exactly once. Both `bendChainUntilMs` (rolling-score) and `bendUntilMs` (triangle) `delete()`d the moment the override fires. Prevents accidental cross-phrase chaining when the next phrase's anchor lands within ±1 of the prior bend's target during the grace window.
- **D62.2 (REVERTED in D63.1)** — `BEND_MAX_SEMI = 12` cap converted bends > 12 semis into leaps. Wild gliss at full 36–89 register lost most of its bending activity → user objected. Reverted.
- **D63** — D60.2's 50 ms bend-dur margin left a 50 ms stationary "still note" tail between bend's atomic transition and the next event. Visible at extreme pitches that clamped to `ROLL_MAX_MIDI = 84`. Fix:
  - D63.1: revert D62.2 (`PITCHBEND_RANGE_SEMI` direct dispatch threshold).
  - D63.2: `BEND_DUR_MARGIN_MS = 5` (was 50). Bend ramp fills inter-event time; residual gap ~2 px on screen.
  - D63.3: `inst.bendPending` + `completeBend(inst, scheduled)` race-safe helper. `glissStep` entry calls `completeBend(inst, false)` to force-complete pending transition inline if scheduling jitter exceeds the 5 ms margin.
- **D64** — bridge `PITCHBEND_RANGE_SEMI` MUST EXACTLY MATCH SWAM preset's Master Tuning → Pitchbend Range. MIDI pitchbend is a 14-bit wheel position with no semitone information; preset does the conversion. Mismatch silently produces audible bend ≠ visual bend → "leaping" perception even when the bridge is bending correctly. The user had multiple sessions where the preset had reverted to default ±2 while the bridge expected ±48 → 24× weaker bends. Fixes:
  - D64.1: `PITCHBEND_RANGE_SEMI = 24` to match user's preset.
  - D64.2: `bang()` logs `=== BRIDGE PITCHBEND_RANGE_SEMI = ±N — verify this matches SWAM preset's Pitchbend Range ===` on every reload. Visible mismatch enforcement.
