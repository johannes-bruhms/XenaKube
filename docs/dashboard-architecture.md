...
- `js/spectrum-score.js` — optional actual-audio spectrogram canvas behind the MIDI brush layer. Consumes only `spectrumFrame` WS data from Max, buffers frames by audio timestamp, applies shared visual latency/nudge, switches declarative color modalities by complex, clears visible history when timing/look settings would reinterpret old pixels, and warns on missing/stale/out-of-order frames. Stale frames scroll existing history but do not paint new spectrum columns. Exports `init` / `setEnabled` / `setScrollSpeed` / `setTiming` / `updateState` / `handleFrame` / `getCanvas`.
...
- **Background visual layers** (`#rolling-score` z-index -1, `#spectrogram-score` z-index -2): MIDI brushes draw above the optional actual-audio spectrogram. Both layers are independently toggleable from the bottom-right `layers` row; default is MIDI on, spectrogram off. When spectrogram is on, `spec ms` applies the same visual delay to MIDI brushes and spectrum frames, and `spec d` nudges spectrum timing for calibration; nudge changes clear buffered spectrum metadata because the audio-time mapping changes. Right edge = visual now, scrolling left at `ROLL_PX_PER_SEC` CSS-px/sec (default 360, retunable live via the `score` slider). Pitch/frequency axis uses `ROLL_TOP_INSET_PX = 70` / `ROLL_BOTTOM_INSET_PX = 80`.
...
`#cube-contrast-layer` is a transparent SVG above `#cube-canvas` and below the K/C-to-sieve line overlay. `cube-scene.js` projects the same 3D endpoints and writes only white structural `<line>` strokes into it (live outer edge, live tetra lines, ghost outer edge, and K↔C line), while CSS applies `mix-blend-mode: difference` per stroke. This is the cheap adaptive-color path: each wire segment inverts whatever the browser has already composited underneath it, including rolling-score strokes and live-cube pixels under the ghost wireframe, without sampling DOM pixels in a shader.
