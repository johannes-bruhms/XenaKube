...
- `js/transport.js` (Phase 2.3) — WebSocket transport. `connect()` / `send(obj)` / `on(name, fn)`. Auto-reconnect at 2 s. Typed events include state/gyro, algorithms, phrase audit, `midiEcho`, `spectrumFrame`, and `spectrumStatus`. Outbound browser `gyro` mirrors are dropped under WS `bufferedAmount` pressure; live `move` messages are still attempted.
...
- `js/state-ui.js` (Phase 2.8) — overlay panels: state rows, mode badges, K/C cards, perm slots, algorithm buffer + toast, scramble bar, expression panel, recent-moves FIFO. Reads `FACE_SIG` and `paintFaceGlyph` from `js/face-glyph.js` for the text face row and active-card glyph. Owns `COMPLEX_SHORT`. Exports `init({ onPathToggle })` / `update(state, move)` / `setCosmologyBadge(cosmology)` / `handleAlgorithmEvent` / `setSolvedBadge(solved, pulse)` / `updateExpression(quat, dev)` / `setAlgorithmBook`.
- `js/main.js` (Phase 2.9) — entry point. Imports every module, calls each one's `init()`, subscribes transport events to module update entry points, wires sliders + buttons + the visible alpha/beta cosmology toggle (`set_mode.cosmology`) + rolling layer toggles + spectrogram settings panel + cube colors panel + Web Bluetooth GAN cube connection, owns the raw GAN `U'`×4 zero-gyro shortcut, dispatches `midi_echo` to sieve + triangle + rolling-score in one place, and forwards spectrum frames only to `spectrum-score`.
...
- **Background visual layers** (`#rolling-score` z-index -1, `#spectrogram-score` z-index -2): MIDI brushes draw above the optional actual-audio spectrogram. Both layers are independently toggleable from the bottom-right `layers` row; default is MIDI on, spectrogram off. A `spectrogram settings` button in the same bottom-right cluster opens the tuning panel for visual delay, audio/MIDI nudge, global gain/floor/ceiling setters, background brightness, shared modality 0.00 color, interpolation density, canvas blur, temporal smoothing, palette override, per-modality gain/floor/ceiling, and every non-background C1-C8 modality palette stop. Modality comes from the spectrum frame's complex; relay fills unknown frame complex from audio-side MIDI noteon timing rather than engine-state timing. Nudge changes clear buffered spectrum metadata because the audio-time mapping changes; look and palette changes clear visible spectrum history so old columns are not mixed with a new interpretation. Right edge = visual now, scrolling left at `ROLL_PX_PER_SEC` CSS-px/sec (default 360, retunable live via the `score` slider). Pitch/frequency axis uses `ROLL_TOP_INSET_PX = 70` / `ROLL_BOTTOM_INSET_PX = 80`.
- **Cube colors panel** (`#cubeColorsPanel`): bottom-right toggleable panel parallel to spectrogram settings. It persists K/C vertex colors, label color/follow-vertex behavior, active/detail label colors, live/ghost/K-C/tetra/adaptive wire colors, top markers, active halos, effective adaptive wire widths, and triangle K/C/sieve leg colors, width, endpoint dot size, and opacity. The panel writes only through `cube-scene.js` / `triangle.js` appearance setters; geometry, snap state, and MIDI line lifetimes remain owned by their original modules.
